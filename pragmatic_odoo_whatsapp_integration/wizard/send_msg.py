        ...
        if not self.env.context.get('default_recipients') and active_model and hasattr(self.env[active_model],
            ...
            if active_model != 'res.partner':
                ...
                if not is_attachment_exists:
                    attachments = []
                    if active_model == 'sale.order':
                        template = self.env.ref('pragmatic_odoo_whatsapp_integration.email_template_sale_new')
                    elif active_model == 'account.move':
                        # template = self.env.ref('account.email_template_edi_invoice')
                        template = self.env.ref('pragmatic_odoo_whatsapp_integration.email_template_invoice_new')
                    elif active_model == 'purchase.order':
                        template = self.env.ref('pragmatic_odoo_whatsapp_integration.email_template_purchase_new')
                        # if self.env.context.get('send_rfq', False):
                        # template = self.env.ref('purchase.email_template_edi_purchase')
                        # else:
                        #     template = self.env.ref('purchase.email_template_edi_purchase_done')
                        ...
                        template = self.env.ref('pragmatic_odoo_whatsapp_integration.mail_template_data_payment_new')
                        # template = self.env.ref('account.mail_template_data_payment_receipt')
            ...
            if active_model == 'sale.order':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    msg = "مرحبًا " + rec.partner_id.name
                    if rec.partner_id.parent_id:
                        msg += "(" + rec.partner_id.parent_id.name + ")"
                    msg += "\n\n ،،نأمل أن تكون خدماتنا حازت على رضاك"
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        msg += "\n\n" + "(" + rec.name + ")" + " ونود إشعارك بعرض الأسعار الخاص بك برقم"
                        # if rec.origin:
                        # msg += "(with reference: " + rec.origin + ")"

                        if rec.date_order:
                            msg += "\n\n" + ".(" + str(rec.date_order.date()) + ")" + " الذي تم إنشاؤه بتاريخ"

                        msg += "\n\n" + " بقيمة إجمالية " + str(rec.amount_total) + ".ريـــال"
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        for line_id in rec.order_line:
                            msg += "\n\n*الوصف:* " + line_id.product_id.name + "\n*الكمية:* " + str(
                                line_id.product_uom_qty) + " " + str(
                                line_id.product_uom.name) + "\n*سعر الوحدة:* " + str(
                                line_id.price_unit) + "\n*المجموع الفرعي:* " + str(line_id.price_subtotal)
                            msg += "\n------------------"
                            ...
                            # msg+="\n------------------"
                    msg += "\n مرفق لك مستند الفاتورة والذي سيساعدك في الحصول على معلومات مفصلة"
            ...
            if active_model == 'account.move':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        # if rec.origin:
                        #     msg += "(with reference: " + rec.origin + ")"
                        msg += "\n\n" + "(" + rec.name + ")" + " ونود إشعارك بالفاتورة الخاصة بك برقم"
                        msg += "\n\n" + " بقيمة إجمالية " + str(rec.amount_total) + ".ريـــال"
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        for line_id in rec.invoice_line_ids:
                            msg += "\n\n*الوصـف:* " + line_id.product_id.name + "\n*الكمية:* " + str(
                                line_id.quantity) + "\n*سعر الوحدة:* " + str(
                                line_id.price_unit) + "\n*المجموع الفرعى:* " + str(line_id.price_subtotal)
                            msg += "\n------------------"
                    msg += "\n مرفق لك مستند أمر البيع والذي سيساعدك في الحصول على معلومات مفصلة"
                    # msg += "\n Please find attached document which will help you to get detailed information."
            ...
            if active_model == 'purchase.order':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        # if rec.origin:
                        #     msg += "(with reference: " + rec.origin + ")"
                        msg += "\n\n" + "(" + rec.name + ")" + " ونود إشعارك ب أمر الشراء الخاص بك برقم"
                        msg += "\n\n" + " بقيمة إجمالية " + str(rec.amount_total) + ".ريـــال"
                        # msg += " with Total Amount " + self.format_amount(rec.amount_total, rec.currency_id) + "."
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        msg += "\n\n.فيما يلي تفاصيل طلبك"
                        for line_id in rec.order_line:
                            msg += "\n\n*الوصــف:* " + line_id.product_id.name + "\n*الكمية:* " + str(
                                line_id.product_qty) + " " + str(line_id.product_uom.name) + "\n*سعر الوحدة:* " + str(
                                line_id.price_unit) + "\n*المجموع الفرعى:* " + str(line_id.price_subtotal)
                            msg += "\n------------------"
                    # msg += "\n Please find attached document which will help you to get detailed information."
                    msg += "\n مرفق لك مستند أمر الشراء والذي سيساعدك في الحصول على معلومات مفصلة"
            ...
            if active_model == 'account.payment':
                if rec.partner_id.mobile and rec.partner_id.country_id.phone_code:
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        msg += "\n\n" + "(" + rec.name + ")" + " ونود إشعارك ب أمر الدفع الخاصة بك برقم"
                        msg += "\n\n" + " بقيمة إجمالية " + str(rec.amount) + ".ريـــال"
                    ...
                    if self.env['ir.config_parameter'].sudo().get_param(
                        ...
                        # for line_id in rec.order_line:
                        msg += "\n\n*نوع الدفع:* " + rec.payment_type + "\n*قيد الدفع:* " + rec.journal_id.name + "\n*تاريخ الدفع:* " + str(
                            rec.payment_date) + "\n*تـذكير:* " + str(rec.communication)
                    msg += "\n مرفق لك مستند أمر الدفـع والذي سيساعدك في الحصول على معلومات مفصلة"
                    # msg += "\n Please find attached document which will help you to get detailed information."
                    if res_user_id.has_group('pragmatic_odoo_whatsapp_integration.group_invoice_enable_signature'):
                        user_signature = self.cleanhtml(res_user_id.signature)
                        msg += "\n\n" + user_signature
